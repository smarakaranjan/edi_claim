...
def get_elem(loop_code, seg_name, pos):
    """
    Fetch the top-level element by segment and position.
    Only consider elements with parent=None (i.e., not sub-elements).
    """
    filters = {
        "segment__name": seg_name,
        "position": pos,
    ...
    if loop_code:
        filters["segment__loop__code"] = loop_code
    else:
        filters["segment__loop__isnull"] = True  # envelope elements

    try:
        return EDIElement.objects.get(**filters)
    except EDIElement.DoesNotExist:
        print(f"⚠️ Element not found: {seg_name} at position {pos} in loop {loop_code}")
        return None
    except EDIElement.MultipleObjectsReturned:
        print(f"⚠️ Multiple elements found for {seg_name}{pos} in loop {loop_code}, returning the first one")
        return EDIElement.objects.filter(**filters).first()


def create_rules(loop_code, rules):
    """
    rules = list of tuples (segment_name, element_position, constant_value)
    If constant_value is a list, treat it as a composite and assign to sub-elements.
    """
    loop_obj = EDILoop.objects.get(code=loop_code) if loop_code else None
    ...
    for seg_name, pos, val in rules:
        ...
        if isinstance(val, list):
            # Composite element: assign values to sub-elements
            for idx, sub_val in enumerate(val, start=1):
                try:
                    sub_elem = EDIElement.objects.get(segment=elem.segment, parent=elem, position=idx)
                except EDIElement.DoesNotExist:
                    print(f"⚠️ Sub-element {seg_name}{pos}-{idx} not found, skipping")
                    continue
                EDIPayerRule.objects.create(
                    payer=payer,
                    loop=loop_obj,
...
create_rules(None, [
    ...
    ("ISA", 5, "ZZ"), ("ISA", 6, "JSDJFRUIFLGHWYH"), ("ISA", 7, "ZZ"), ("ISA", 8, "MSRYHTRGKDLERNV"),
    ("ISA", 9, "260111"), ("ISA", 10, "1200"), ("ISA", 11, "U"), ("ISA", 12, "00501"),
...
create_rules("1000A", [
    ...
    # ("REF", 1, "1G"),  # REF01 – Reference Identification Qualifier (example)
    # ("REF", 2, "SUBMITTERID"),  # REF02 – Reference Identification value
...
# create_rules("2000B", [
#     ("HL", 1, "2"),  # This HL ID
#     ("HL", 2, "1"),  # Parent HL ID (from 2000A)
#     ("HL", 3, "22"), # Level Code
#     ("SBR", 1, "P"), ("SBR", 2, "18"),
# ])
...
create_rules("2000B", [
    ...
    ("HL", 2, "1"),   # Parent HL ID (from 2000A)
    ("HL", 3, "22"),  # Level Code
    ("SBR", 1, "P"),  # Subscriber Relationship
    ("SBR", 2, "18"), # Insurance Type Code
...
create_rules("2300", [
    # Claim Information
    ("CLM", 1, "CLM0001"),   # Claim Number
    ("CLM", 2, "150.00"),    # Claim Total Amount
    ("CLM", 3, "Y"),         # Claim Submission Type
    ("CLM", 4, "INFO123"),
    ("CLM", 5, ["11", "B", "1"]),  # composite
    ("CLM", 6, "Y"),

    # Claim Dates
    ("DTP", 1, "434"),       # Date Qualifier (Claim)
    ("DTP", 2, "D8"),        # Date Format
    ("DTP", 3, "20260101"),  # Claim Date

    ("DTP", 1, "472"),       # Another Claim Date (if needed)
    ("DTP", 2, "D8"),
    ("DTP", 3, "20260101"),
    ...
    # Claim Reference Identification
    ("REF", 1, "1K"),        # Reference ID Qualifier
    ("REF", 2, "12345"),
...
create_rules("2400", [
    ...
    ("SV1", 1, "HC:99213:25"),  # Composite Procedure Code
    ("SV1", 2, "100.00"),       # Line Item Charge Amount
    ("SV1", 3, "UN"),           # Unit or Basis for Measurement
    ("SV1", 4, "1"),            # Service Unit Count
    ("SV1", 5, "11"),           # Place of Service Code
    ("SV1", 6, "1"),            # Service Type Code

    # DTP – Service Line Date
    ("DTP", 1, "472"),          # Service Date Qualifier
    ("DTP", 2, "D8"),           # Date Format
    ("DTP", 3, "20260101"),     # Service Date

    # REF – Service Line Additional Identification
    ("REF", 1, "6R"),           # Reference ID Qualifier
    ("REF", 2, "LINE1")         # Reference ID
])
...
create_rules("2010AA", [
    ...
    ("NM1", 1, "85"),            # Entity Identifier Code
    ("NM1", 2, "2"),             # Entity Type Qualifier
    ("NM1", 3, "ACME CLINIC"),  # Last Name / Organization Name
    ("NM1", 9, "123456789"),     # Billing Provider Identifier
    ...
    # REF – Additional Identification (optional)
    ("REF", 1, "EI"),            # Reference Identification Qualifier
    ("REF", 2, "123456789"),     # Reference Identification
])
...
create_rules("2010BA", [
    # NM1 – Subscriber Name
    ("NM1", 1, "IL"),          # Entity Identifier Code
    ("NM1", 2, "1"),           # Entity Type Qualifier (1 = Person)
    ("NM1", 3, "DOE"),         # Last Name
    ("NM1", 4, "JOHN"),        # First Name
    ("NM1", 5, ""),            # Middle Name
    ("NM1", 6, ""),            # Name Prefix
    ("NM1", 7, ""),            # Name Suffix
    ("NM1", 8, "MI"),          # Identification Code Qualifier
    ("NM1", 9, "123456789"),   # Subscriber ID
    ("N3", 1, "789 PATIENT LN"), 
    ("N3", 2, ""),
    ("N4", 1, "PATIENTCITY"), 
    ("N4", 2, "TX"), 
